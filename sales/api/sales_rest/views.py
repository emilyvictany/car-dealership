...
def api_salesperson(request):
    ...
    elif request.method == "DELETE":  # delete a specific salesperson
        try:
            salesperson = Salesperson.objects.get(employee_id=id).delete()
            return JsonResponse({"confirmation": "Salesperson has been deleted"})
        except Salesperson.DoesNotExist:
...
def api_sales(request):
    if request.method == "GET":  # list all sales
    ...
    elif request.method == "DELETE":  # delete a specific sale
        try:
        ...
        try:
            ...
            return JsonResponse(
                {"message": "Automobile is not available"},
                status=404
            )
        try:
